...
export async function registerRoutes(app: Express) {
  app.get("/api/weather", async (req, res) => {
    try {
      ...
      const response = await axios.get(
        `https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lng}&exclude=minutely,hourly,alerts&units=metric&appid=${WEATHER_API_KEY}`
      );
  ...
  app.get("/api/distance", async (req, res) => {
    try {
      ...
      const distance = distanceResponseSchema.parse(response.data.rows[0].elements[0]);
      res.json(distance);
    } catch (error) {
      res.status(400).json({ error: "Invalid request" });
    }
